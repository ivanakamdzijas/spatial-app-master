...
public interface City40MongoRepo extends MongoRepository<City40Mongo, String> {
	
	City40Mongo findByCityAndCountry(String city, String country);
	
	/*@Query(value = "{geom: {$nearSphere: {$geometry:  ?0, $maxDistance: ?1 } }}")
	List<City10Mongo> getAllOnDistanceFromBg(Point geom, int km);*/
	List<City40Mongo> findByGeomNear(Point geom, Distance  d);
	

	@Query(value = "{\"country\":\"Russia\", geom: {$geoIntersects: {$geometry: ?0}}}")
	 List<City40Mongo> getEuropeRussia(GeoJsonMultiPolygon europe);
	...
	List<City40Mongo> getEuropeCities(GeoJsonMultiPolygon europe);
	
	@Override
	<S extends City40Mongo> S save(S city);
